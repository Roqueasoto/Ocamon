...
let state_same st st' b = (*ignore (failwith "AAA"); print_endline "XXX"; *)
  let effect = fun () ->
    if b then
      begin
        if st = st' then print_endline "SAME" else print_endline "DIFF"
      end
    else
      () in
  effect (); st'

let get_b l1 l2 = let open Controller in
  (l1 = [Nothing] && l2 = [Nothing])

let print_b b = (if b then print_endline "YYY2" else ())
      ...
      if not is_success then (st, false)
        ...
        let (poke_self', poke_other') =
          ...
          | Other -> begin
              ...
              let poke_other' = Pokemon.poke_effect state_info.poke_other eff in
...
(* Requires that elist be expanded.
  ...
  let do_round user_elist enemy_elist st =
    ...
    let st' = state_same st st' b in (* TODO DELETE*)
