...
let press_poke () = 
  ...
  while !keep_running do 
    ...
    then  
      if s.Graphics.key = '1' 
      then (poke_num := 0; keep_running := false)
      else 
        if s.Graphics.key = '2'
        then (poke_num := 1; keep_running := false)
        else 
            if s.Graphics.key = '3'
            then (poke_num := 2; keep_running := false)
            else keep_running := true
    else keep_running := true;
    done; !poke_num 
...
let start_three () = 
  ...
  moveto 110 60;
  draw_string "Press 1, 2, or 3";
  moveto 110 40;
  draw_string "to choose your first OCAMON!";
  set_font "-misc-dejavu sans mono-bold-r-normal--12-0-0-0-m-0-iso8859-1";
  moveto 280 20;
  draw_string "Press 1, 2, or 3 to continue. > ";
...
let get_cmd gui_inf gmode =
  ...
  | MQuit -> failwith "ROQUE IS A LIAR"
