...
let draw_battle gui_inf = 
  ...
  moveto 80 70;
  draw_string ("1. " ^ (List.assoc 2 (action_names (snd (List.hd (comb_inf.user_person_info.poke_inv))))));
  moveto 80 100;
...
let rec press_map () =
  let keep_running = ref true in 
    while !keep_running do 
      ...
      then  
        if s.Graphics.key = 'c' 
        then keep_running := false (*or close_graph()*)
        else 
          keep_running := true
    done 
...
let draw_start () = 
  ...
  moveto 80 150;
  draw_string "You have three pokemon to choose from!"; 
  moveto 80 120;
  draw_string "Press 'a', 's', or 'd' to start game.";
...
let get_cmd gui_inf gmode =
  let start () = 
    ...
    Interact (CStart n) in (* this is the number of the pokemon that they pick 0,1,2*) 
  match gui_inf.mode with
  (* | MStart -> Interact (CStart (draw_start ())) *)
  | MStart -> start()
  | MMap -> failwith "GUI MAP MODE"; draw_map (); 
    Interact CMap
